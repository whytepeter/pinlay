...
const reporterHue = computed(() => display.value.faviconHue);
</script>
...
<template>
  ...
  >
    ...
    <div class="flex flex-col gap-2.5 p-4 pl-5">
      ...
      <span
        v-if="session.board"
        class="inline-flex w-fit items-center gap-1.5 rounded-full bg-muted px-2 py-0.5 text-[11px] font-medium text-foreground"
        :title="`Board: ${session.board.name}`"
      >
        <span
    ...
    >
      <UserAvatar :name="reporterName" :hue="reporterHue" :size="20" />
      <span class="text-xs font-medium text-foreground">{{
        firstName(reporterName)
      }}</span>
