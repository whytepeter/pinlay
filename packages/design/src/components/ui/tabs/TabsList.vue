...
<template>
  <TabsList
    ...
    :class="cn(
      // iOS segmented control: recessed pill track, raised pill thumb (below).
      // Track and thumb share the full radius so the geometry always nests.
      'scrollbar-hide relative inline-flex w-fit shrink-0 items-center gap-0.5 overflow-auto rounded-full border border-transparent bg-muted p-1 text-muted-foreground data-[orientation=vertical]:w-full data-[orientation=vertical]:flex-col data-[orientation=vertical]:items-stretch',
      props.class,
    )"
  >
    <div
      aria-hidden="true"
      class="pointer-events-none absolute left-0 top-0 rounded-full bg-background shadow-sm ring-1 ring-black/[0.04] transition-all duration-300 ease-out dark:bg-muted-foreground/20 dark:ring-white/[0.06]"
